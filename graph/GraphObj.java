...
abstract class GraphObj extends Graph {
    ...
    public int edgeSize() {
        ...
        if (isDirected()) {
            ...
            int specialEdge = 0;
            for (int key : unDirectedHash.keySet()) {
                for (int elem : unDirectedHash.get(key)) {
                    if (elem == key) {
                        specialEdge += 1;
                    } else {
                        totlEdSz += 1;
    ...
    public void remove(int v) {
        if (contains(v)) {
            if (isDirected()) {
                ...
                ArrayList<Integer> lst = unDirectedHash.get(v);
                if (lst.contains(v)) {
                    lst.remove((Integer) v);
                }
                for (int elem : lst) {
                    unDirectedHash.get(elem).remove((Integer) v);
                }
                unDirectedHash.remove((Integer) v);
            }
        }
    }
