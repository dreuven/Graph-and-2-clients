...
public abstract class Traversal {
    ...
    protected Traversal(Graph G, Queue<Integer> fringe) {
        _G = G;
        _fringe = fringe;
        visitedList = new HashSet<Integer>();
    }
        ...
        while (!_fringe.isEmpty()) {
            ...
            if (!marked(vertex)) {
                ...
                if (reverseSuccessors(vertex)) {
                    ...
                    for (int succ : _G.successors(vertex)) {
                        if (processSuccessor(vertex, succ)) {
                            tempStack.push(succ);
                        }
                        while (!tempStack.isEmpty()) {
                            _fringe.add(tempStack.pop());
                        }
                    }
                } else {
                    for (int succ : _G.successors(vertex)) {
    ...
    /** A set useful in checking if a node has been visited. */
    protected HashSet<Integer> visitedList;
    /** List of PostVisited. */
    protected ArrayList<Integer> myPVisit = new ArrayList<Integer>();
}
