...
class Rule {
    ...
    void addCommands(List<String> commands) {
        try {
            if (_commands.size() != 0 && commands.size() != 0) {
                error("CommandSet non-empty and input non-empty");
            }
        } catch (IllegalStateException excp) {
            error("Illegal CommandSet and command sizes, both are Zero!");
        }
    ...
    private void checkFinishedDependencies() {
        for (int succ : _depends.successors(getVertex())) {
            if (!_depends.getLabel(succ)._finished) {
                error("ERROR, invalid Dependencies");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        for (int succ : _depends.successors(getVertex())) {
            if (_depends.getLabel(succ).getTime() == null
                || _depends.getLabel(succ).getTime() > getTime()) {
                return true;
            }
        }
        return false;
